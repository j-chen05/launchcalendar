...
class Launchlib:
    ...
    """
    search(self, location, agency)
        - Search through the most recently fetched launch data with specified parameters, and return a list
        of matches, if any
            ...
            if location != None and agency != None:
                if location in result_loc and agency in result_ag:
                    obj = Launch_obj(result['name'], result['pad']['location']['name'],
                                     result['mission']['description'], result['window_start'], result['window_end'])
